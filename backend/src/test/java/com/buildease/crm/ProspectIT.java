...
class ProspectIT {
  ...
  Actor actor(String email) {
    var tokens = auth.login(email, password, "prospect-" + email);
  ...
  @Test
  void prospectMovesThroughTheFunnelAndLeasingRequiresApprovalFirst() {
    ...
    prospects.updateStatus(
        s.owner(), s.organization(), s.building(), prospect, ProspectStatus.SCREENING, null);
    prospects.updateStatus(
    ...
    assertThat(detail).containsEntry("status", "APPROVED");
  }

  @Test
  void linkingALeaseRequiresApprovalAndMovesTheProspectToLeased() {
    Setup s = organizationWithVacantSpace();
    UUID prospect =
        prospects.create(
            s.owner(),
    ...
    var detail = prospects.detail(s.owner(), s.organization(), s.building(), prospect);
    assertThat(detail).containsEntry("status", "LEASED").containsEntry("lease_id", lease);
  }

  @Test
  void statusCannotChangeOnceInATerminalState() {
    Setup s = organizationWithVacantSpace();
    UUID prospect =
        prospects.create(
            s.owner(),
  ...
  @Test
  void nonManagerCannotCreateOrAdvanceProspects() {
    Setup s = organizationWithVacantSpace();
  ...
  private UUID draftLease(Setup s) {
