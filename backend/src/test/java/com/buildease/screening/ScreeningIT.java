...
import com.buildease.crm.ProspectService;
import com.buildease.crm.ProspectStatus;
...
class ScreeningIT {
  ...
  @Autowired ScreeningService screenings;
  ...
  Actor actor(String email) {
    var tokens = auth.login(email, password, "screening-" + email);
  ...
  @Test
  void requestingAScreeningMovesAnAppliedProspectIntoScreeningAndRecordsThePassResult() {
    ...
    var prospectDetail = prospects.detail(s.owner(), s.organization(), s.building(), prospect);
    assertThat(prospectDetail).containsEntry("status", "SCREENING");

    var results = screenings.list(s.owner(), s.organization(), s.building(), prospect);
    assertThat(results).hasSize(1);
    assertThat(results.getFirst()).containsEntry("status", "PASS");
    assertThat((String) results.getFirst().get("report")).contains("Jane Prospect");
  }

  @Test
  void cannotScreenAProspectThatHasNotApplied() {
    Setup s = organizationWithVacantSpace();
    UUID prospect =
        prospects.create(
            s.owner(),
  ...
  @Test
  void nonManagerCannotRequestOrViewScreenings() {
    Setup s = organizationWithVacantSpace();
    UUID prospect =
        prospects.create(
            s.owner(),
    ...
    assertThatThrownBy(() -> screenings.request(tenant, s.organization(), s.building(), prospect))
        .isInstanceOfSatisfying(ApiException.class, e -> assertThat(e.status).isEqualTo(403));
    assertThatThrownBy(() -> screenings.list(tenant, s.organization(), s.building(), prospect))
        .isInstanceOfSatisfying(ApiException.class, e -> assertThat(e.status).isEqualTo(403));
  }
}
