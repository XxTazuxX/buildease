...
class OnlinePaymentIT {
  ...
  @Autowired OnlinePaymentService payments;
  ...
  Actor actor(String email) {
    var tokens = auth.login(email, password, "online-payment-" + email);
  ...
  private Setup organizationWithActiveLease() {
  ...
  @Test
  void residentCanPayTheirOwnActiveLeaseAndTheChargeAppearsOnTheLedger() {
    Setup s = organizationWithActiveLease();
    Actor resident = actor(s.residentEmail());

    payments.pay(resident, s.organization(), s.building(), s.lease(), new BigDecimal("500.00"));

    var detail = leases.lease(s.owner(), s.organization(), s.building(), s.lease());
    assertThat((BigDecimal) detail.get("balance")).isEqualByComparingTo("-500.00");
    @SuppressWarnings("unchecked")
    var paid = (java.util.List<java.util.Map<String, Object>>) detail.get("payments");
    assertThat(paid).hasSize(1);
    assertThat(paid.getFirst()).containsEntry("method", "CARD");
  }

  @Test
  void aResidentCannotPaySomeoneElsesLease() {
    Setup a = organizationWithActiveLease();
    Setup b = organizationWithActiveLease();
    Actor residentA = actor(a.residentEmail());

    assertThatThrownBy(
            () ->
                payments.pay(
                    residentA, b.organization(), b.building(), b.lease(), new BigDecimal("500.00")))
        .isInstanceOfSatisfying(ApiException.class, e -> assertThat(e.status).isEqualTo(403));
  }

  @Test
  void cannotPayADraftOrEndedLease() {
    Setup s = organizationWithActiveLease();
    Actor resident = actor(s.residentEmail());
    leases.end(
        s.owner(),
